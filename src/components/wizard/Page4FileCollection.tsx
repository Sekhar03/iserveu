    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#e2e8f0]">
          ...
          {/* Right side corner: Download Sample File */}
          <div className="flex items-center gap-2 self-start sm:self-center">
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
                exportSampleFile('xlsx', `sample_${activeFile.id}_format`);
              }}
              className="px-3.5 py-2 rounded-xl bg-white hover:bg-[#119db0]/10 border border-[#119db0]/40 text-[#0e8696] font-semibold text-xs transition-all flex items-center gap-2 shadow-sm hover:shadow cursor-pointer group"
              title="Download sample format template (.xlsx) with exact parameters"
            >
              <Download className="w-4 h-4 text-[#119db0] group-hover:-translate-y-0.5 transition-transform" />
              <span>Download Sample File</span>
              <span className="text-[10px] bg-[#119db0]/15 text-[#0e8696] font-bold px-1.5 py-0.5 rounded">.xlsx</span>
            </button>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
                exportSampleFile('csv', `sample_${activeFile.id}_format`);
              }}
              className="px-2.5 py-2 rounded-xl bg-white hover:bg-slate-100 border border-slate-200 text-slate-600 font-semibold text-xs transition-all flex items-center gap-1 shadow-sm hover:shadow cursor-pointer"
          ...
          {currentFileState.status === 'pending' && (
            ...
            >
              ...
              <p className="text-xs text-[#475569] mt-1">
                Supports .csv, .xlsx files ({activeFile.channel})
              </p>
